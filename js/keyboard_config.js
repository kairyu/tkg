...
function initKeyboardConfigPanel(name) {
	if (name == "Kimera") {
		...
		$($row_input.tagsinput('input')).attr('lang', $row_input.attr('lang'));
		$($col_input.tagsinput('input')).attr('lang', $col_input.attr('lang'));
...
function kimeraRowMappingChange() {
	var $row_input = $('#kbd-cfg-container #kimera-row-val');
	var input_ports = _.map($row_input.val().split(','), function(val) {
		return parseInt(val);
	});
...
function kimeraColMappingChange() {
	var $col_input = $('#kbd-cfg-container #kimera-col-val');
	var input_ports = _.map($col_input.val().split(','), function(val) {
		return parseInt(val);
	});
